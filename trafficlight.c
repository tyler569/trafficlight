...
long millisecond_now() { return nanosecond_now() / 1000000; }
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv) {
	...
	SDL_Window *window
		= SDL_CreateWindow("Traffic Light", SDL_WINDOWPOS_UNDEFINED,
	...
	SDL_Surface *sdl_surface = SDL_CreateRGBSurface(0, renderer_width,
		renderer_height, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0);
...
int stage_id(struct light_spec *spec, int offset, long time) {
	...
	for (struct light_stage *stage = spec->stages; stage->state; stage++) {
		if (loop_time >= stage[0].time_offset
